 ...
 *
 * All seeded entries carry source="seed" so they can be surgically removed
 * without touching real captures.
 *
 * Server URL: MEMOIR_API env var, defaults to http://localhost:3000
 */

const API = process.env.MEMOIR_API ?? 'http://localhost:3000';
const NODE_TLS = process.env.NODE_TLS_REJECT_UNAUTHORIZED;
...
async function deleteSeedEntries() {
  const res = await fetch(`${API}/api/entries?limit=10000&source=seed`);
  const entries = (await res.json()) as { id: string }[];
  for (const e of entries) {
    await fetch(`${API}/api/entries/${e.id}`, { method: 'DELETE' });
  }
  console.log(`Deleted ${entries.length} seeded entries.`);
}
...
async function main() {
  ...
  if (reset || clean) await deleteSeedEntries();
  if (clean) return;
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (!res.ok) { console.error(`\nFailed entry ${i}: ${res.status}`); continue; }
